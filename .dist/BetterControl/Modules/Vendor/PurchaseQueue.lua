...
function Utils.GetRawStock(item)
	...
	local _, _, _, stackCount, numAvailable = GetMerchantItemInfo(item.index)
...
function Utils.GetAffordableQuantity(item)
	...
	local limit = item.maxPerRequest or GetMerchantItemMaxStack(item.index) or unitSize
	...
	if hasCurrencyCost then
		if CanAffordMerchantItem and not CanAffordMerchantItem(item.index) then
			return 0
		end
...
function Utils.BuildExtendedCosts(index)
	local costs = {}
	local totalCosts = GetMerchantItemCostInfo(index) or 0
	for costIndex = 1, totalCosts do
		local texture, amount, link, currencyName = GetMerchantItemCostItem(index, costIndex)
		costs[#costs + 1] = {
			texture = texture,
			amount = amount or 0,
			link = link,
...
function Utils.BuildVendorItem(index)
	...
	return {
		...
		price = price or 0,
		unitSize = unitSize,
		maxPerRequest = GetMerchantItemMaxStack(index) or unitSize,
